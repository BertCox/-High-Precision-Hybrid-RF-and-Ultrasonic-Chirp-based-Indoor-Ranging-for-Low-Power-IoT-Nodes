function [run_once, n_sources]=roomsim_run(config_file, plots);
% Usage: [run_once, n_sources]=roomsim_run();
...
%% 
%% modified by ir. H. Brouckxon, Vrije Universiteit Brussel, dept ETRO-DSSP
%% 
%% revision history: 
%%      * 2007.01: added "config_file" parameter, the simulation now by
%%                 default gets its parameters through this text file.
%%      
%%                 
%% 
...
% **************************************************************
% * read the parameter values from the configuration text file *
% **************************************************************
...
if plots
    ...
    disp('Showing roomplot: press key to continue')
    pause
end
...
if plots
    ...
    disp('Showing roomplot absorption: press key to continue')
    pause
end
...
if max(max(B))==0, % Anechoic case, set H_length and order to ensure FULL CODE PATH EXECUTED in all following functions incl. Plots.
    ...
    if plots
        ...
        disp('Showing roomplot reverb time: press key to continue')
        pause
    end
...
cd('..\RIR'); % Change directory to the Impulse_response directory (folder)
for ps=1:n_sources % For each parent source
    imp_file_name=[H_filename '.mat']; % Compose the impulse response filename for primary source ps
