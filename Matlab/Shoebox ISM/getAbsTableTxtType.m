function [surface_data, F_abs]=getAbsTableTxtType(type);
...
% Type is the keyword that will be used for the appropriate selection
%---------------------------------------------------------------------------- 
...
%%Edited Bart Thoen
%%Get data from list based on typename

% Compose pathname to absorption data file
abs_root ='step_1_Calculate_RIR\roomsim\Text_setups';
...
selection = ismember(surface_list,type);

if max(selection) > 0
    surface_data = data(selection,:);% Return surface absorption values as Column vectors.
else
    surface_data = [0.5	0.7 0.6 0.7 0.7 0.5];
    warning(['No reference to keyword ' type ' found, absorbtion set to acoustic_tile_suspended']);
end
